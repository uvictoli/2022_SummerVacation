...
#install package - #update r version()
if(!(installed.packages() %in% c("instalr","rstudiopapi")))
  install.packages("instalr",dependencies=T)
install.packages(c("instalr","rstudioapi"), dependecncies=T)
library(installr)
library(rsudioapi)
rstCheck<-check.for.updates.R()
if(rskChek==TRUE)
  updateR()

# Prepreperation
getwd()
setwd("/Users/yooseungli/Downloads/데청캠/오프라인 수업/220711/chapter1")
installed.packages("rstudioapi",dependencies=T)
library(rstudioapi)
...
#class(anscombe) # data type 확인

# Export Dataframe to excel
install.packages("writexl", dependencies = T)
library(writexl)
write_xlsx(anscombe, "anscombe.xlsx")

# Show summaries X
with(anscombe, summary(x1))
with(anscombe, summary(x2))
with(anscombe, summary(x3))
with(anscombe, summary(x4))

# Show summaries Y
with(anscombe, summary(y1))
with(anscombe, summary(y2))
with(anscombe, summary(y3))
with(anscombe, summary(y4))
...
# place four plots in ine page using standard R graphics
